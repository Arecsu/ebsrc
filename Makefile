...
CC65FLAGS = -t none --cpu 65816 -I include -I src --include-dir include
...
extract:
	./tools/ebbinex "earthbound.yml" "donor.sfc"

extractproto:
	./tools/ebbinex "earthbound-1995-03-27.yml" "donor-1995-03-27.sfc"

extractjp:
	./tools/ebbinex "mother2.yml" "donorm2.sfc"

$(BUILDDIR)/%.o: $(SRCDIR)/%.asm
	@$(call mkdir, $(@D))
	ca65 $(CA65FLAGS) --listing "$(strip $(subst $(SRCDIR), $(BUILDDIR), $(patsubst %.o,%.lst,$@)))" -o "$@" "$<"

# C compilation rules - simple cc65 to ASM
src/%.s: src/%.c
	@$(call mkdir, $(@D))
	cc65 $(CC65FLAGS) -o "$@" "$<"
